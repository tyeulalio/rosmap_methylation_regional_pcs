...
#process_chrom <- function(chrom){
    ...
    #datafile <- paste0("../../output/05_qtl_analysis/04_mishap_test/chrom", chrom, "_hg38.missing.hap")
...
#exclude_all <- lapply(chroms, process_chrom)
#exclude_df <- do.call(rbind, exclude_all) %>%
    #unique()
head(exclude_df)
dim(exclude_df)
...
#test <- read_delim("../../output/qtl_analysis/06_merge_bims/all_chroms_hg38.fam", col_names=FALSE, delim=' ')
#head(test)
#length(intersect(test$X2, remove_samples$specimenID))
