...
gwas_type="wightman"
savedir <- paste0("/path/to/new_rosmap/output/07_ptwas/05_lifted_ld_panel/", gwas_type, "/")
dir.create(savedir)
...
datafile <- paste0("/path/to/new_rosmap/output/07_ptwas/04_formatted_gwas/", gwas_type, "_gwas_file.vcf.gz")
...
process_chromosome <- function(chromosome){ 
    ...
    ld_file <- paste0("/path/to/data/G1K_EUR_3V5/chr", chromosome, ".vcf.gz")
    ...
    chain <- import.chain("/path/to/data/hg19ToHg38.over.chain")
