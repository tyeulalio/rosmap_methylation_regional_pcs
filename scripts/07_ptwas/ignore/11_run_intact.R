...
savedir <- paste0("/home/eulalio/deconvolution/new_rosmap/output/07_ptwas/11_intact/")
dir.create(savedir)
savedir <- paste0("/home/eulalio/deconvolution/new_rosmap/output/07_ptwas/11_intact/", gwas_type, "/")
dir.create(savedir)
plot_dir <- paste0(savedir, "plots/")
dir.create(plot_dir)
    

load_ptwas <- function(runname, gwas_type){
    # load ptwas data
    datafile <- paste0("/home/eulalio/deconvolution/new_rosmap/output/07_ptwas/07_ptwas_scan/", gwas_type, "/with_proportions/",
...
load_fastenloc <- function(runname, gwas_type){
    ...
    datafile <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/04_fastenloc/01_fastenloc_output/with_proportions/",
...
main <- function(){
    ...
    process_run <- function(row){
        ...
        #if (file.exists(savefile)){
            #print(paste("Loading existing results file"))
            #res <- read_tsv(savefile)
            #return(res)
        #}
