...
savedir <- paste0("/home/eulalio/deconvolution/new_rosmap/output/07_ptwas/04_formatted_gwas/")
dir.create(savedir)
...
if (gwas_type == 'wightman'){
    ...
    if (!file.exists(savefile)){
        gwas_file <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/02_gwas_prep/02_check_liftover/",
                            gwas_type,
        ...
        test_kunkle <- function(){
            gwas_file <- paste0("/home/eulalio/deconvolution/new_rosmap/output/06_fine_mapping_colocalization/02_gwas_prep/02_check_liftover/",
                                gwas_type, "gwas_updated_scores.csv")#, "/matched_chrom_pos_mapped_to_variant_name.tsv")
            #datafile <- paste0("/home/eulalio/deconvolution/data/ad_gwas/Kunkle_etal_2019_Stage1_results.txt")
        ...
        datafile <- paste0("/home/eulalio/deconvolution/data/ad_gwas/wightman_etal_2021_results.txt.gz")
        full_gwas <- read_table(datafile)
        head(full_gwas)
    ...
    datafile <- paste0("/home/eulalio/deconvolution/new_rosmap/data/rosmap_wgs_liftover_hg38/MAFs/rosmap_wgs_hg38_all_chroms_MAFs.txt")
    ...
    #final_list <- list(MAF=final_gwas$MAF,
                       #snp=final_gwas$snp,
                       #position=final_gwas$position,
                       #type='cc',
                       #s=0.11,
                       #N=762917,
                       #pvalues=final_gwas$pvalues
    #)
    ...
    check_qtl <- FALSE
    if (check_qtl){
        qtl_file <- paste0("/home/eulalio/deconvolution/new_rosmap/output/07_ptwas/03_gambit_db/with_proportions/avgs_bulk_full_gene/all_gene.ptwas_weights.gambit.vcf")
...
check_qtl <- FALSE
if (check_qtl){
    qtl_file <- paste0("/home/eulalio/deconvolution/new_rosmap/output/07_ptwas/03_gambit_db/all_gene.ptwas_weights.gambit.vcf.gz")
    qtl_weights <- read_table(qtl_file, comment="##")
    head(qtl_weights)
