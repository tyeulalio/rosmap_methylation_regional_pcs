...
correlate_umap <- function(formatted_clinical, cv_savedir, datatype=""){
    targets <- clinical
    head(targets)
    dim(targets)
    ...
    #pc_file <- paste0(cv_savedir, precleaned_str, datatype, cell_type, "_global_pcs.rds")
        ...
        #pc_file <- paste0(cv_savedir, cleaned_str, precleaned_str, datatype, cell_type, "_global_pcs.rds")
    #}
    #pcs <- readRDS(pc_file)
    #head(pcs)
    ...
    pcs <- umap_data %>%
        as.data.frame() %>%
        rownames_to_column('individual_celltype') %>%
        separate(individual_celltype, c('individualID', 'cell_type'), remove=FALSE, sep='_', extra='merge')
    head(pcs)

    #ordered_samples <- intersect(unique(pcs$individualID), target_traits$individualID)
    ...
    for (pc_num in 1:n.pc){
        for (trait_num in 1:n.traits){
            ...
            pc_name <- paste0("UMAP_", pc_num)
            if (class(pc_df[,trait]) != 'numeric'){
                pvals_mat[pc_num, trait_num] <- kruskal.test(pc_df[,pc_name] ~ pc_df[,trait])$p.value
            } else{
                pvals_mat[pc_num, trait_num] <- cor.test(pc_df[,pc_name], pc_df[,trait], method='spearman',
                                                         exact=FALSE)$p.value
            }
    ...
    pvals_long <- pvals_mat %>%
        as.data.frame() %>%
        mutate(pc=paste("UMAP", row_number(), sep='_')) %>%
